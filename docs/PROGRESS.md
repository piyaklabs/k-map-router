...
### 4차 테스트 결과 (2026-06-11, 버전 9812e65b)
- ✅ 데스크톱 카카오 **출발지+도착지 정상** (WCongnamul rt= 방식 동작 확인). 데스크톱 paste도 정상.
- ✅ iOS Paste 말풍선 안내 동작. 단 **말풍선 눌러도 빈 문자열 반환되는 iOS WebKit 버그 실측**
  → readText 1회 재시도 + 그래도 비면 "long-press → Paste" 수동 안내로 수정 (수동 경로는 정상 동작 확인됨).
...
- **paste 재오류 수정**: ① 4s 타임아웃 race 제거(커밋 687570d). ② 진짜 원인은 **버튼이 `read()`를 먼저
  호출 → iOS Safari에서 `getType('text/uri-list')` 실패 → 빈 값 → readText 폴백은 제스처 소비로 막힘**
  = 버튼 죽음. 네이티브 길게눌러-붙여넣기는 정상이라 텍스트는 클립보드에 있음을 확인 → **readText()를
  먼저** 호출하고 read()는 빈 값일 때만 폴백으로 변경(버전 419da759). 말풍선 1회로 끝.
